...
export function Logo({ to = '/', size = 'md', variant = 'dark', className = '' }: LogoProps) {
    ...
    <Link to={to} className={`inline-flex items-center gap-2 ${className}`}>
      <svg width={mark} height={mark} viewBox="0 0 32 32" fill="none" aria-hidden="true">
        <path
          ...
          fill={variant === 'light' ? '#ffffff' : '#4f46e5'}
        />
        <path
          d="M11 16.2 14.5 19.7 21.5 12.5"
          stroke={variant === 'light' ? '#4f46e5' : '#ffffff'}
          strokeWidth={2.4}
          strokeLinecap="round"
          strokeLinejoin="round"
        />
      </svg>
      <span className={`${text} font-semibold tracking-tight ${textColor}`}>{APP_NAME}</span>
    </Link>
  )
}
